...
import { certData, knowledge, sideData } from '../../data/data';
...
export const SecondPage: React.FC = () => {
	return (<Page className={styles.root}>
			<Section title="Logros y certificaciones">
				{certData.map((cert) => (<SectionItem title={cert.title} info={cert.info} key={cert.title} />))}
			</Section>
			<Section title="Otros proyectos">
			...
			<TagSection title="Aptitudes y conocimientos">
				{knowledge.map((item) => (<PropElement key={item.title}>{item.title}</PropElement>))}
			</TagSection>
		</Page>);
};
